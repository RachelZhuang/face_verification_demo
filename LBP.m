...
if (nargin > 2) && (length(varargin{2}) == 1)
    ...
    %spoints是neighbors的坐标构成的矩阵,第一个为y坐标，第二个为x坐标，默认中间的那个点坐标为（0,0）
    spoints=zeros(neighbors,2);

    % Angle step.
    a = 2*pi/neighbors;
    
    %默认中间的那个点坐标为（0,0），x轴向右为正方向，y轴向下为正方向
    for i = 1:neighbors
...
%spoints是neighbors的坐标构成的矩阵，neighbors是周围点的个数
...
% Block size, each LBP code is computed within a block of size  bsizey*bsizex(没弄明白)
bsizey=ceil(max(maxy,0))-floor(min(miny,0))+1;
bsizex=ceil(max(maxx,0))-floor(min(minx,0))+1;

% Coordinates（坐标） of origin (0,0) in the block
